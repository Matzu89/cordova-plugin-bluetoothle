cordova.define("cordova-plugin-bluetoothle.BluetoothLEPlugin", function (require, exports, module) {
  ...
  console.log("INITIALIZING WATCH TO NULL");
  var watcher = null;

  module.exports = {
    initialize: function (successCallback, errorCallback, params) {
      // Deteremine if bluetooth is present and enabled
      radios.Radio.getRadiosAsync().done(function (result) {
        enabled = false;
        var bluetoothRadioExists = false;

        console.log(result);

        result.forEach(function (radio) {
          if (radio.kind === radios.RadioKind.bluetooth) {
            bluetoothRadioExists = true;
        ...
        if (!bluetoothRadioExists) {
          errorCallback({ error: "initialize", message: "Bluetooth not supported." });
        } else {
          initialized = true;
          successCallback({ status: (enabled ? "enabled" : "disabled") });
        }

      }, function (error) {
        errorCallback({ error: "initialize", message: error.toString() });
      });
    },

    startScan: function (successCallback, errorCallback, params) {
      if (!checkInitializedAndEnabled(errorCallback)) {
        return;
      }

      if (watcher) {
        errorCallback({ error: "startScan", message: "Scanning already in progress" });
        return;
      }
      ...
      // Used so we don't try to look up one device while we're in the process of looking up another
      var lookupPromise = new WinJS.Promise.wrap();

      // Scan for Bluetooth LE advertisements.
      ...
      watcher.onreceived = function (eventArgs) {
        // Convert the Bluetooth address into the form we want to provide the user (xx:xx:xx:xx:xx:xx)
        if (eventArgs.advertisementType === 4) {
          var macAddress = eventArgs.bluetoothAddress.toString(16).match(/(..)/g).join(":");

          if (!devices[macAddress]) {
            devices[macAddress] = { rawAddress: eventArgs.bluetoothAddress };
            var result = {
              status: "scanResult",
            ...
            /*lookupPromise = lookupPromise.then(function () {
             return Windows.Devices.Bluetooth.BluetoothLEDevice.fromBluetoothAddressAsync(eventArgs.bluetoothAddress).then(function (bluetoothDevice) {
             devices[macAddress] = bluetoothDevice;
             ...
             console.log("Error getting bluetooth device from address " + eventArgs.bluetoothAddress + ":");
             console.log(error);
             });
             });*/
          }
    ...
    stopScan: function (stopScanSuccess, stopScanError) {
      if (!checkInitializedAndEnabled(stopScanError)) {
        return;
      }

      if (!watcher) {
        stopScanError({ error: "stopScan", message: "Not scanning" });
        return;
      }

      watcher.onstopped = function () {
        console.log("Watcher stopped");
        watcher.onreceived = null;
        watcher.onstopped = null;
        watcher = null;
        stopScanSuccess({ status: "scanStopped" });
      }
    ...
    connect: function (successCallback, errorCallback, params) {
      ...
      var address = params && params.length && params[0].address;
      if (!address) {
        errorCallback({ error: "connect", message: "No device address" });
        return;
      }

      var device = devices[address];
      if (!device) {
        errorCallback({ error: "connect", message: "Device not found" });
        return;
      }

      if (device.isConnected) {
        errorCallback({ error: "connect", message: "Device previously connected, reconnect or close for new connection" });
        return;
      }

      Windows.Devices.Bluetooth.BluetoothLEDevice.fromBluetoothAddressAsync(device.rawAddress).then(function (bluetoothLEDevice) {
        console.log("bluetoothLEDevice:");
        console.log(bluetoothLEDevice);
        device.wasPaired = bluetoothLEDevice.deviceInformation.pairing.isPaired;
        if (device.wasPaired) {
          device.isConnected = true;
          device.id = bluetoothLEDevice.deviceId;
          // We are already paired with this device - should be good to proceed.
          console.log("ALREADY PAIRED");
          successCallback({ status: "connected", "name": bluetoothLEDevice.name, address: address });
        } else {
          console.log("NO PAIRED");
          // We need to pair to do anything
          console.log("PAIRING...");
          bluetoothLEDevice.deviceInformation.pairing.pairAsync().done(function (result) {
            device.isConnected = true;
            device.id = bluetoothLEDevice.deviceId;
            ...
            console.log(result);
            errorCallback({ error: "connect", message: error.message });
          });
        }
      }, function (error) {
        errorCallback({ error: "connect", message: error.message });
    ...
    services: function (successCallback, errorCallback, params) {
      ...
      if (!device.isConnected) {
        errorCallback({ error: "neverConnected", message: "Never connected to device" });
        return;
      }

      Windows.Devices.Bluetooth.BluetoothLEDevice.fromBluetoothAddressAsync(device.rawAddress).then(function (bluetoothLEDevice) {
        console.log("Got bluetoothLEDevice:");
        console.log(bluetoothLEDevice);
      }, function (error) {
      ...
      /*console.log("DEVICE ID:");
       console.log(device.id);
       var selector = "System.Devices.ContainerId:={" + device.id + "} AND System.Devices.InterfaceClassGuid:=\"{6E3BB679-4372-40C8-9EAA-4509DF260CD8}\" AND System.Devices.InterfaceEnabled:=System.StructuredQueryType.Boolean#True";
      ...
      // Scan paired devices until we find the one that matches
      /*var selector = "System.Devices.InterfaceClassGuid:=\"{6E3BB679-4372-40C8-9EAA-4509DF260CD8}\" AND System.Devices.InterfaceEnabled:=System.StructuredQueryType.Boolean#True";
       ...
       var matchString = "_" + address.split(":").join("") + "#";
       console.log("matchString:" + matchString);
       devices.some(function (device) {
       console.log("device.id: " + device.id);
       console.log("Matches? " + (device.id.indexOf(matchString) > -1));
       if (device.id.indexOf(matchString) > -1) {
       matchingDevice = device;
       return true;
       }
       return false;
       });
       console.log("MATCHING DEVICE:");
       console.log(matchingDevice);
       }, function (error) {
       console.log("ERROR FINDING PAIRED DEVICES:");
       console.log(error);
       }).then(function () {
       // Unpair
       console.log("Trying to unpair");
       Windows.Devices.Bluetooth.BluetoothLEDevice.fromBluetoothAddressAsync(device.rawAddress).then(function (bluetoothLEDevice) {
       console.log("Is paired: bluetoothLEDevice.deviceInformation.pairing.isPaired");
       bluetoothLEDevice.deviceInformation.pairing.unpairAsync().then(function (result) {
       console.log("UNPAIRED!");
       console.log(result);
       }, function (error) {
       console.log("DIDN'T UNPAIR!");
       console.log(error);
       });
       }, function (error) {
       errorCallback({ error: "connect", message: error.message });
  ...
  function checkInitializedAndEnabled(errorCallback) {
    return checkInitialized(errorCallback) && checkEnabled(errorCallback);
  }

  function checkInitialized(errorCallback) {
    if (!initialized) {
      errorCallback({ error: "initialize", message: "Bluetooth not initialized" });
      return false;
    }
    return true;
  }

  function checkEnabled(errorCallback) {
    if (!enabled) {
      errorCallback({ error: "enable", message: "Bluetooth not enabled" });
      return false;
    }
    return true;
  }
