...
module.exports = {
  ...
  initialize: function (successCallback, errorCallback, params) {
    ...
    if (radios) {
      // This is Windows 10 or later, and we have access to real information about whether Bluetooth is supported
      // and enabled, so use that.

      if (bluetoothRadio) {
        // Clear out our current bluetoothRadio and look for a new one (handle scenarios where a bluetooth dongle
        // might have been disconnected and our radio is no longer valid).
        bluetoothRadio.onstatechanged = null;
        bluetoothRadio = null;
      }

      radios.Radio.getRadiosAsync().done(function (radioList) {
        enabled = false;

        // Iterate until we find the first enabled bluetooth radio
        enabled = radioList.some(function (radio) {
        ...
        if (!bluetoothRadio) {
          ...
          var isStatusReceiver = params.statusReceiver !== false; // keepCallback
          if (isStatusReceiver) {
            bluetoothRadio.onstatechanged = function (eventArgs) {
              enabled = eventArgs.target.state === radios.RadioState.on;
              successCallback({status: (enabled ? "enabled" : "disabled")}, {keepCallback: true});
            };
          }
          ...
          initialized = true;
          successCallback({status: (enabled ? "enabled" : "disabled")}, {keepCallback: isStatusReceiver});
        }

      }, function (error) {
        errorCallback({error: "initialize", message: error.message || error});
      });
    } else {
        ...
        if (service.getAllCharacteristics) { // Phone
          ...
          if (serviceInfo) {
            ...
            errorCallback({
              ...
              message: "Unknown service, add the service to getServiceInfos() and try again."
            });
          }
        }
        successCallback({
